...
with DAG(
    dag_id="gbq_daily_intermediate_b01",
    # schedule_interval=None,
    schedule_interval="00 05 * * *",
    start_date=dt.datetime(2022, 6, 1),
    catchup=True,
    max_active_runs=1,
    tags=['convz', 'testing', 'mario', 'intermediate'],
        ...
        if iterable_tables_list:
            for index, table_fqdn in enumerate(iterable_tables_list):
                ...
                with TaskGroup(
                    ...
                    for interval in range(DATE_START,DATE_END):
                            ...
                            configuration = {
                                "query": {
                                    ...
                                    "destinationTable": {
                                        ...
                                        "tableId": f'test_{tm1_table}${{{{ ti.xcom_pull(task_ids="gen_date_{DATASET_DST}.{tm1_table}_{interval}").replace("-","") }}}}',
                                    },
