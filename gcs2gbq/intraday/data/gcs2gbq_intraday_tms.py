...
def _generate_schema(table_name, report_date, run_date):
    ...
    schema.append({"name":"run_date", "type":"DATETIME", "mode":"REQUIRED"})

    query = f"{query}\tDATETIME('{report_date}') AS `report_date`,\n"
        ...
        for line in lines:
            ...
            if split_line[1] == 0:
                ...
                if timestamp == str(runtime):
                    ...
                    log.info(f"Adding file [{split_line[2]}] with timestamp {timestamp} and size {split_line[1]} byte to list.")
...
def _check_xcom(table_name, tm1_varible):
    if len(tm1_varible) == 0:
        log.info(f"Table [ {table_name} ] has no intraday file to load.")
...
with DAG(
    ...
    catchup=True,
    max_active_runs=1,
    tags=['convz', 'production' ,'mario', 'intraday_data', 'tms', 'gcs'],
    ...
    # iterable_tables_list = [ "tms_tbdeliverycontrol" ]
        ...
        if iterable_tables_list:
            for index, tm1_table in enumerate(iterable_tables_list):
                ...
                create_schema = PythonOperator(
                    ...
                    op_kwargs={ 
                        'table_name' : tm1_table,
                        'report_date': '{{ ts.replace(".","+").split("+")[0] }}',
                    ...
                    configuration = {
                        "load": {
                            ...
                            "destinationTable": {
                                "projectId": PROJECT_ID,
                                "datasetId": f"{DATASET_ID}_stg",
                                "tableId": f'{tm1_table}_{SOURCE_TYPE}_stg${{{{ ts.split(":")[0].replace("-","").replace("T","") }}}}'
                            },
                ...
                load_final = BigQueryInsertJobOperator( 
                    ...
                    configuration = {
                        "query": {
                            ...
                            "writeDisposition": "WRITE_TRUNCATE",
                            "useLegacySql": False,
                            "timePartitioning": { "field":"report_date", "type":"HOUR" },
                        }
                    }
                )
